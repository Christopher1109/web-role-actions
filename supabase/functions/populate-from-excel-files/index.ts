...
// Estructura completa de insumos por tipo de anestesia (extraídos de los Excel)
const dataExcelByType = {
  sedacion: [
  ...
  loco_regional: [
    ...
    "MIDAZOLAM. SOLUCIÓN INYECTABLE CADA AMPOLLETA CONTIENE: CLORHIDRATO DE MIDAZOLAM EQUIVALENTE A 5 MG DE MIDAZOLAM O MIDAZOLAM 5 MG ENVASE CON 5 A",
    "FENTANILO. SOLUCIÓN INYECTABLE CADA AMPOLLETA O FRASCO ÁMPULA CONTIENE: CITRATO DE FENTANILO EQUIVALENTE A 0.5 MG DE FENTANILO. ENVASE CON 6 AMPOLLETAS O FRASCOS ÁMPULA CON 10 ML.",
    "ONDANSETRÓN. SOLUCIÓN INYECTABLE CADA AMPOLLETA O FRASCO AMPULA CONTIENE: CLORHIDRATOD DIHIDRATADO DE ONDANSETRÓN EQUIVALENTE A 8 MG DE ONDANSETRÓN ENVASE CON 3 AMPOLLETAS O FRASCOS ÁMPULA CON 4 ML.",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("Iniciando proceso de poblado de insumos desde Excel...");

    // Primero, cargar todos los insumos existentes para evitar duplicados
    ...
    if (insumosExistentes) {
      insumosExistentes.forEach((insumo: any) => {
    ...
    // Recopilar todos los nombres únicos de todos los tipos de anestesia
    const todosLosNombres = new Set<string>();
    Object.values(dataExcelByType).forEach(lista => {
      lista.forEach(nombre => todosLosNombres.add(nombre));
    });

    console.log(`Total de nombres únicos en archivos Excel: ${todosLosNombres.size}`);

    // Insertar los insumos que no existen
    ...
    for (let i = 0; i < insumosNuevos.length; i += BATCH_SIZE) {
      ...
      if (error) {
        console.error("Error insertando insumos:", error);
        continue;
      }

      if (nuevosCreados) {
        nuevosCreados.forEach((insumo: any) => {
          mapaNombresAIds.set(insumo.nombre, insumo.id);
        });
      }

      console.log(`Batch ${Math.floor(i / BATCH_SIZE) + 1}: Creados ${batch.length} insumos`);
    }

    console.log(`Total de insumos en mapa después de inserts: ${mapaNombresAIds.size}`);

    // Ahora poblar la tabla anestesia_insumos
    const estadisticas: Record<string, number> = {};

    for (const [tipoAnestesia, nombres] of Object.entries(dataExcelByType)) {
      console.log(`Procesando tipo: ${tipoAnestesia} con ${nombres.length} insumos`);
      
      // Obtener relaciones existentes para este tipo
      const { data: relacionesExistentes } = await supabaseClient
        .from("anestesia_insumos")
        .select("insumo_id")
        .eq("tipo_anestesia", tipoAnestesia);

      const insumoIdsExistentes = new Set(
        relacionesExistentes?.map((r: any) => r.insumo_id) || []
      );

      console.log(`Relaciones existentes para ${tipoAnestesia}: ${insumoIdsExistentes.size}`);

      // Preparar inserts de relaciones que no existen
      ...
      for (let i = 0; i < nombres.length; i++) {
        ...
        if (!insumoId) {
          console.warn(`No se encontró ID para insumo: ${nombre}`);
          continue;
        }

        if (!insumoIdsExistentes.has(insumoId)) {
      ...
      console.log(`Relaciones nuevas a crear para ${tipoAnestesia}: ${relacionesNuevas.length}`);

      // Insertar relaciones en lotes
      if (relacionesNuevas.length > 0) {
        for (let i = 0; i < relacionesNuevas.length; i += BATCH_SIZE) {
          ...
          if (error) {
            console.error(`Error insertando relaciones para ${tipoAnestesia}:`, error);
          } else {
            console.log(`Batch insertado: ${batch.length} relaciones para ${tipoAnestesia}`);
          }
        }
      }

      estadisticas[tipoAnestesia] = nombres.length;
    }
    ...
    const resumen = {
      ...
      insumosCreados: insumosNuevos.length,
